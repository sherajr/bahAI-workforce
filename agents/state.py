...
def init_db():
    ...
    with _connect() as conn:
        ...
        for col in ("image_prompt TEXT", "theme TEXT",
                    ...
                    # How this product's quotation was verified, and against
                    # what (rule 111). Stored so an old word-overlap pass can
                    # never be read as though it had passed today's exact check
                    # -- and so an automated regeneration cannot lean on a
                    # verified flag whose method is unknown.
